...
- `findPath`
  ...
  Аргументы:
  * `NavigationRequest schema` - схема, содержащая точки начала и конца
...
#### GraphNode
Класс, представляющий узел внутри графа.

Доступные методы:
- `getNavNode/setNavNode` - метод получения/изменения соответствующего узла из базы данных
- `getPosition` - получить положение узла в виде `Vector2`
- `getConnections` - получить узлы `GraphNode`, с которыми данный узел связан
- `getParentNode` - получить родительский узел
...
Доступные приватные поля:
- `List<GraphNode> nodes` - доступные узлы в графе
- `Map<NavNode, GraphNode> navNodeByGraphNode` - таблица для поиска `GraphNode` по соответствующему `NavNode`
- `Map<GraphNode, NavNode> graphNodeByNavNode` - таблица для поиска `NavNode` по соответствующему `GraphNode`
- `Map<GraphNode, GraphNode> distanceMatrix` - матрица расстояний между узлами. При создании графа заполняется
  расстояниями между соседними(соединенными) узлами
